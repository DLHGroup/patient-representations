...
if __name__ == "__main__":
    #Test files
    for xmi_test_file in os.listdir(os.path.dirname(sys.path[0]) + '/Comorbidity/Test_BATCH_OUT/'):        
        if xmi_test_file[-3:]=="xmi":
            xmi_file = os.path.dirname(
                sys.path[0]) + '/Comorbidity/Test_BATCH_OUT/' + xmi_test_file
            #print(xmi_file)
    ...
    #Train files
    for xmi_train_file in os.listdir(os.path.dirname(sys.path[0]) + '/Comorbidity/Train_BATCH_OUT/'):
        if xmi_train_file[-3:] == "xmi":
            xmi_file = os.path.dirname(
                sys.path[0]) + '/Comorbidity/Train_BATCH_OUT/' + xmi_train_file
            #print(xmi_file)
            tree = ET.parse(xmi_file)
            root = tree.getroot()
            dictionary_polarity = {}
