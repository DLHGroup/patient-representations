...
note_events_file = os.path.dirname(
    sys.path[0]) + '/Codes/MimicIII/Source/NOTEEVENTS.csv'

dict_notes = {}
print('just before the opened notes file')
with open(note_events_file, newline='') as file_name:
    print('inside the opened notes file')
...
for subject in dict_notes:
    i += 1
    path = sys.path[0] + '/MimicIII/MIMIC_IN/'#+str(i % 30+1)+'/'
    print(path)
